...
function tvRP.CarregarObjeto(del,dict,anim,prop,flag,mao,pos1,pos2,pos3,xa,ya,za)
	local player = PlayerPedId()

	if del == nil then del = true end
	delObjet = del

	-- ANIMACAO
	if dict then
		tvRP.CarregarAnim(dict)
		TaskPlayAnim(player, dict, anim, 4.0, 1.0, -1, flag, 0, 0, 0, 0 ) 
	end
	-- OBJETO
	...
	object = CreateObject(GetHashKey(prop), x, y, z+0.2,  true,  true, true)
	AttachEntityToEntity(object, player, GetPedBoneIndex(player, mao), pos1,pos2,pos3,xa,ya,za, true, true, false, true, 1, true)
...
function tvRP.getObjeto()
	return object
end
function tvRP.DeletarObjeto(forceDelete)
	if (forceDelete or delObjet) and DoesEntityExist(object) then
