...
local function chest_create(bau_id, owner_id,stype,sid,cid,config,data,x,y,z,player)
	...
	if user_id and (user_id == owner_id or vRP.hasPermission(user_id,"policia.permissao")) then
		vRP.openChest2(player,"u"..bau_id.."-"..config.name.."home",config.weight, "", nil, nil, nil)
	end
end
...
local function wardrobe_create(bau_id, owner_id,stype,sid,cid,config,data,x,y,z,player)
	...
	if user_id and user_id == owner_id then
		...
		local sets = json.decode(udata) or {}
...
local function wardrobe_destroy(bau_id, owner_id,stype,sid,cid,config,data,x,y,z,player)
	local nid = "vRP:home:slot"..stype..sid..":wardrobe"
	vRPclient._removeNamedMarker(player,nid)
	vRP.removeArea(player,nid)
end
