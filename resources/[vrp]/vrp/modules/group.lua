...
function vRP.addUserGroup(user_id,group)
	if not vRP.hasGroup(user_id,group) then
		...
		if ngroup then
			...
			TriggerClientEvent("Notify",source,"sucesso","Add com sucesso do grupo:"..group)
		end
	end
end
...
function vRP.removeUserGroup(user_id,group)
	...
	if groupdef then
		...
		TriggerClientEvent("Notify",source,"importante","Removido com sucesso do grupo:"..group)
	end
end
