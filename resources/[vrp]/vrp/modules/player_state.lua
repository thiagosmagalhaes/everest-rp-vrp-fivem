...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
    ...
    if first_spawn then
        if data.colete then vRPclient.setArmour(source, data.colete) end
        ...
        if data.position then
            vRPclient.teleport(source, data.position.x, data.position.y,
                               data.position.z)
        end
...
function tvRP.updateArmor(armor)
    ...
    if user_id then
        local data = vRP.getUserDataTable(user_id)
        if data then data.colete = armor end
    end
end
...
function tvRP.setNocauteado(nocauteado)
    local user_id = vRP.getUserId(source)
    local nocautes = vRP.getUserNocautes(user_id)
    if user_id then
        if not nocauteado then
            nocautes.time = false
        else
            nocautes.time = os.time()
        end
    end
end

function tvRP.getNocauteado(cb)
    local user_id = vRP.getUserId(source)
    if user_id then
        local time = vRP.getUserNocautes(user_id)
        if time.time then
            return os.time() - time.time
        else
            return false
        end
    else
...
AddEventHandler("trymala",
                function(nveh) TriggerClientEvent("syncmala", -1, nveh) end)
