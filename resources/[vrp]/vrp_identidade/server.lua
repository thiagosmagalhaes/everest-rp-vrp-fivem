...
function vRPN.Identidade()
	...
	if user_id then
		...
		if identity then
			return identity.foto,identity.name,identity.firstname,identity.user_id,identity.registration,identity.age,identity.phone,vRP.format(parseInt(mymultas)),vRP.format(parseInt(mypaypal)),groupv
		end
	end
end
