...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        $(".info-div").html(event.data.capacidade);
        ...
        $('.item').draggable({
            ...
            stop: function () {
                itemData = $(this).data("item");
                $(this).css('background-image', 'url(\'img/items/' + itemData.nome + '.png\'');
...
function closeInventory() {
    $.post("http://vrp_inventario/NUIFocusOff", JSON.stringify({}));
}
...
function inventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        $("#playerInventory").append('<div class="slot"><div id="item-' + index + '" class="item" style = "background-image: url(\'img/items/' + item.nome + '.png\')">' +
...
function secondInventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        $("#otherInventory").append('<div class="slot"><div id="itemOther-' + index + '" class="item" style = "background-image: url(\'img/items/' + item.nome + '.png\')">' +
            '<div class="item-count">' + count + '</div> <div class="item-name">' + item.label + '</div> </div ><div class="item-name-bg"></div></div>');
    ...
    $('#give').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                ...
                // $.post("http://vrp_inventario/GetNearPlayers", JSON.stringify({
                //     item: itemData
                // }));
            }
        }
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                $.post("http://vrp_inventario/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                $.post("http://vrp_inventario/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
