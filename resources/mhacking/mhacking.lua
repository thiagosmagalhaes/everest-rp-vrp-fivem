	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navegue através das teclas ~y~W,A,S,D~s~ e para confirmar pressione ~y~SPACE~s~.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Use ~y~Arrow Keys~s~ e ~y~ENTER~s~ para fixar os códigos.")
			else
